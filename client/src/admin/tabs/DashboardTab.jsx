  ...
  export default function DashboardTab({ students, setStudents, setActiveTab, onViewDetails, settings }) {
    ...
    useEffect(() => {
      ...
      if (isWithin(now, firstSemStart, firstSemEnd)) {
        setCurrentSemester("1st Semester");
      } else if (isWithin(now, secondSemStart, secondSemEnd)) {
        setCurrentSemester("2nd Semester");
      } else {
        setCurrentSemester("Outside Semester");
      }
    ...
    useEffect(() => {
      ...
      // 2. Poll until socket is ready (connectSocket is async)
      const interval = setInterval(() => {
        const s = getSocket();
        if (s) {
          setSocketInstance(s);

          s.on("studentUpdated", (updated) => {
            setStudents((prev) =>
              prev.map((stud) =>
                stud.student_id === updated.student_id
                  ? { ...stud, ...updated }
                  : stud
              )
            );
    ...
    const handleClearance = async (student_id) => {
      try {
        ...
        socketInstance?.emit("studentUpdated", { student_id, enrollment_status: 1 });
    ...
    const handleRevokeClearance = async (student_id) => {
      try {
        ...
        socketInstance?.emit("studentUpdated", { student_id, enrollment_status: 0 });
