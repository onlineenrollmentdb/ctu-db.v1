...
import { connectSocket, getSocket, disconnectSocket } from "../../socket"; // ✅ named imports
...
export default function DashboardTab({ students, setStudents, setActiveTab, onViewDetails, settings }) {
  ...
  useEffect(() => {
    ...
    const socketInstance = getSocket(); // get the initialized socket

    socketInstance.on("studentUpdated", (updated) => {
      setStudents((prev) =>
        prev.map((s) => (s.student_id === updated.student_id ? { ...s, ...updated } : s))
      );
    });

    return () => socketInstance.off("studentUpdated");
  }, [setStudents]);
  ...
  const handleClearance = async (student_id) => {
    try {
      ...
      getSocket().emit("studentUpdated", { student_id, enrollment_status: 1 });
  ...
  const handleRevokeClearance = async (student_id) => {
    try {
      ...
      getSocket().emit("studentUpdated", { student_id, enrollment_status: 0 });
