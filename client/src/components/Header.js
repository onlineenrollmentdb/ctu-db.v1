...
// Lazy-load icons
const BellIcon = lazy(() => import("react-icons/bi").then(mod => ({ default: mod.BiBell })));
const HouseIcon = lazy(() => import("react-icons/bi").then(mod => ({ default: mod.BiHome })));
const PencilIcon = lazy(() => import("react-icons/bi").then(mod => ({ default: mod.BiPencil })));
const ChartIcon = lazy(() => import("react-icons/bi").then(mod => ({ default: mod.BiBarChart })));
const LogoutIcon = lazy(() => import("react-icons/bi").then(mod => ({ default: mod.BiBoxArrowRight })));
const MegaphoneIcon = lazy(() => import("react-icons/bi").then(mod => ({ default: mod.BiMegaphone })));
...
const Header = ({ onHome, onEnroll, onGrades, onProfile, onLogout, settings }) => {
  ...
  const fullName = useMemo(() => {
    if (!student) return "";
    return `${student.first_name || ""} ${student.middle_name || ""} ${student.last_name || ""}`.replace(/\s+/g, " ").trim();
  }, [student]);
        ...
        <Suspense fallback={null}>
          ...
          {announcement && (
            <div className="announcement">
              <MegaphoneIcon className="ann-icon"/>
          ...
          {showAnnouncementModal && announcement && (
            <div className="modal-overlay">
              <div className="modal-content" style={{ maxWidth: "400px" }} ref={modalRef}>
                ...
                <p>Starts: <span style={{ color: "var(--primary-color)" }}>{announcement.start.toLocaleDateString()}</span></p>
                <p>Ends: <span style={{ color: "var(--primary-color)" }}>{announcement.end.toLocaleDateString()}</span></p>
              </div>
            </div>
          )}
